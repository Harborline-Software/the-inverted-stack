...
The audit trail is the legally defensible record of when access ended. For employment disputes, it documents when the deployment terminated the former employee's data access. For partnership dissolutions, it is the partition authorization record. For regulated industries — HIPAA, SOX, PCI-DSS, NIST SP 800-12 [11], DIFC DPL 2020, India DPDP Act 2023 + RBI BFSI circular, China PIPL + MLPS 2.0, Japan PIPA, South Korea PIPA + ISMS-P, Russia 242-FZ, NDPR, POPIA — it is the access-termination artifact those frameworks require. The trail composes with the recovery-event audit trail in Ch22 §Key-Loss Recovery sub-pattern 48f; both share the `Sunfish.Kernel.Audit` substrate while their record schemas remain distinct. Cross-reference to §Chain-of-Custody for Multi-Party Transfers (in this chapter) for the multi-party signed-event substrate that underpins both.
...
Microsoft Purview sensitivity labels [41], AWS Macie, and Google Cloud DLP all perform classification and re-classification as centralized, online operations against a canonical store. The architecture's contribution is the operation that does the same job *under partial replication, without duplicating storage, without breaking the immutable audit trail, and without requiring the central store those systems assume*. The mechanism is convergent re-classification — an idempotent CRDT operation that propagates through the same gossip path every other operation uses, with a max-register invariant guaranteeing every replica reaches the same class label regardless of operation arrival order.
