...
Five testing levels govern this architecture. Each carries mandatory pass conditions before first production release; §4 specifies the CI cadence at each level. Each scenario specifies setup conditions, the action to perform, and the expected outcome. If a scenario has no pass condition, it is not a test.
