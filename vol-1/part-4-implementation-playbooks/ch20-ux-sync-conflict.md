...
`Harborline.UIAdapters.Blazor.SunfishFreshnessBadge` renders the per-record freshness indicator for any data class. Bind it to the AP/CP classification configured in `Harborline.Kernel.Sync`; the badge computes staleness against the configured threshold and updates its state without additional application code. Do not hand-roll freshness timers. The stale-window edge cases - clock skew, daylight-saving boundaries, suspend/resume - are handled by the component.
...
<!-- code-check: this section references a forward-looking namespace `Harborline.Kernel.Performance` and pre-1.0 component names (`SunfishMergeProgressIndicator`, `IProgressiveDegradation`, `PerformanceDegraded` `SyncState` sub-state) that are part of the Volume 1 extension roadmap. The other references - `Harborline.UIAdapters.Blazor`, `Harborline.Foundation` - are in the current Harborline Shipyard package canon. -->
...
`Harborline.UIAdapters.Blazor` provides a `SunfishMergeProgressIndicator` component that implements the ambient indicator pattern. Wire it to the `IProgressiveDegradation` contract from `Harborline.Kernel.Performance`. The component handles the three state transitions without additional application code.
...
**Developer-facing.** `Harborline.Kernel.Performance` emits structured telemetry events for every budget violation. In development builds, violations surface in the application diagnostics panel with operation class, actual duration, and declared budget. A developer who is shipping a component that consistently violates the write budget sees the violation immediately, without running a profiler. The diagnostic panel is not visible to end-users in production builds; it is wired only for development and staging environments.
...
<!-- code-check: this section references two forward-looking Harborline namespaces - `Harborline.Foundation.Recovery` and `Harborline.Kernel.Audit` - that are part of the Volume 1 extension roadmap and not yet present in the Harborline Shipyard reference implementation. They are illustrative in the same sense the book's existing pre-1.0 Harborline references are illustrative. `Harborline.Kernel.Security` is in the current Harborline Shipyard package canon. -->
...
<!-- code-check: this section references three Harborline namespaces. `Harborline.Kernel.Security` is in the current Harborline Shipyard package canon. `Harborline.Kernel.Audit` is forward-looking - introduced by extension #48 and extended here for revocation and partition records. `Harborline.Foundation.Recovery` is forward-looking under ADR 0046 and is referenced for the successor-entity KEK separation in the dissolution scenario. The forward-looking namespaces are illustrative in the same sense the book's existing pre-1.0 Harborline references are illustrative. -->
...
`Harborline.Kernel.Security` manages the underlying key rotation and the revocation event publication. Cross-reference to Ch23 §Collaborator Revocation and Post-Departure Partition sub-pattern 45b for the rotation specification.
...
<!-- code-check annotations: Harborline.Kernel.Security (in-canon, surfaces the re-classification policy and the access-control re-evaluation); Harborline.Kernel.Audit (in-canon per cerebrum 2026-04-28, supplies the escalation-event record the operator UI reads); Harborline.Kernel.SchemaRegistry (in-canon, supplies the cross-class reference index the operator review surface queries); Harborline.Kernel.Sync (in-canon, propagates the operation); Harborline.UIAdapters.Blazor (in-canon, hosts the class-indicator component alongside SunfishFreshnessBadge). 0 new top-level namespaces. 0 class APIs / method signatures introduced. -->
...
The class-indicator component lives in `Harborline.UIAdapters.Blazor` alongside the freshness badge described in §AP/CP Visibility by Data Class. Bind it to the record's current class as exposed by `Harborline.Kernel.Security`; the component reads the most recent escalation event from `Harborline.Kernel.Audit` for the recent-change state and updates without additional application code. Do not hand-roll the computation. The escalation-event read path is the same surface the operator console reads, and a parallel implementation will drift.
...
The confirmation dialog names the consequences in plain language: "This will tighten access to [N] records. Roles losing access: [list]. The escalation cannot be reversed; if needed, you can recreate the record at a lower class with a fresh identifier." After confirmation, `Harborline.Kernel.Security` issues the operation, `Harborline.Kernel.Sync` propagates it, and the audit log records the assertion. A progress indicator shows propagation status across the fleet; the operator does not have to wait for full propagation to dismiss the dialog, but the badge on each affected record updates as the operation lands.
...
After the escalation propagates, the reference index in `Harborline.Kernel.SchemaRegistry` identifies records holding inbound references to the escalated record (Ch23 §Event-Triggered Re-classification sub-pattern 10d). Those candidates surface in the operator's review queue as a "References to recently-escalated records" panel - a list with the candidate record, its current class, the escalated record it references, and two actions: "Confirm class unchanged" and "Re-classify upward."
