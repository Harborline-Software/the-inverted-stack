...
<!-- code-check: namespaces inherited from Ch15. Harborline.Kernel.Security, Harborline.Kernel.Sync, Harborline.Kernel.Audit (forward-looking from #48), Harborline.Foundation.Recovery (forward-looking from #48). Per-section code-check annotations carried with their sections. -->

---

A key fails in three ways. It can be compromised - taken from a node that should still hold it. It can be lost - not retrievable by the user who legitimately owns it. It can be stale - outliving a session boundary an adversary has partially observed. Chapter 15 specifies the architecture that holds keys; this chapter specifies the operational flows that run when keys reach those failure states. Each section is a self-contained operational flow; cross-references to Chapter 15 (architectural primitives), Chapter 14 (sync daemon protocol), and Chapter 23 (endpoint, collaborator, and custody operations) point readers to the context they need.
...
<!-- code-check: package references in this section include `Harborline.Foundation.Recovery` (forward-looking - Volume 1 extension roadmap, not yet present in the Harborline Shipyard reference implementation; illustrative in the same sense the book's other pre-1.0 Harborline references are illustrative). The other two references, `Harborline.Kernel.Security` and `Harborline.Kernel.Audit`, are in the current Harborline package canon (verified 2026-04-28: packages/kernel-audit/ exists). -->
...
A sixth failure mode is documented but not this chapter's to solve: vendor unreachability. In 2022, large-scale Western SaaS service terminations across the CIS region - Adobe, Autodesk, Figma, Microsoft - left organisations unable to access keys, files, and project state held inside vendor systems they could no longer reach. For cloud-key-custody architectures, that case is unrecoverable: when the vendor goes silent, so do the keys. The local-first architecture this chapter operates over does not face that scenario at all. User-held keys remain user-held when a vendor terminates service, when a relay becomes unreachable, or when a software publisher exits a market. Recovery primitives in this chapter address the case where the user's own access is broken; vendor-unreachability never engages the recovery primitive, because the user's keys are already on the user's device. That distinction is the load-bearing argument for why the recovery primitives below are worth their attack-surface cost.
...
| Regulated (HIPAA, PCI, financial; DPDP, DIFC DPL, PIPL, Russia 242-FZ) | Custodian-held under attestation | Multi-sig social with named officers | 30 days |
...
Multi-sig social recovery - sub-pattern 48a - distributes the recovery authority across a set of trusted individuals the user designates before any loss occurs. The construction derives from Shamir secret sharing [1]: the user's root recovery key is split into *n* shares, each share held by a separate trustee, and any *t* of the *n* shares suffices to reconstruct the key. Each trustee holds a share, not the full key; no single trustee can unilaterally reconstruct the recovery key or access the user's data.

A threshold of 3-of-5 tolerates the simultaneous unavailability of two trustees; a threshold of 2-of-3 is appropriate for smaller trust networks. The Argent smart wallet specification [2] and Vitalik Buterin's 2021 case for social recovery wallets [3] establish the pattern's practical architecture.
...
Social-recovery legal status varies by jurisdiction. Trustee residency may itself trigger data-residency obligations under the DPDP Act (India), PIPL (China), Japan PIPA, South Korea PIPA, DIFC DPL 2020 (Dubai International Financial Centre) and the parallel ADGM Data Protection Regulations 2021 (Abu Dhabi Global Market), Russia's Federal Law 242-FZ, NDPR (Nigeria), Kenya DPA, POPIA Section 72 (South Africa), Brazil's LGPD, and Mexico's LFPDPPP cross-border transfer requirements. Deployments subject to localization requirements name their trustees in-jurisdiction or document an exception in the team's compliance posture (see Appendix F). For EU-resident users with trustees outside the EU/EEA, the share transfer is itself a data transfer under GDPR Chapter V - deployments name a Chapter V transfer mechanism (Standard Contractual Clauses, an adequacy decision, or binding corporate rules) before designating an out-of-region trustee. After Schrems II (CJEU C-311/18, July 2020), SCCs alone do not provide adequate protection where the importing jurisdiction's surveillance law conflicts with EU fundamental rights; technical and organisational measures are required, and end-to-end encryption with local key custody - exactly the architecture this chapter operates over - is one of the EDPB's named technical measures for closing the gap. The cryptographic construction is sound everywhere; the legal classification of the trustees' role is not.

The deployment cost is low: trustee designation is a setup flow in `Harborline.Foundation.Recovery`. The ongoing cost is maintaining accurate trustee contact information as relationships change.
...
`Harborline.Foundation.Recovery` emits recovery-claim events and manages the grace-period state machine. The grace period is not a client-side timer; it is an event in the signed audit log, which means it is tamper-evident and observable by any node that validates the log.

#### Recovery-event audit trail

Sub-pattern 48f is the logging substrate on which all other mechanisms depend. Every recovery initiation, trustee response, dispute, and completion is a signed event in the same encrypted log used for application data. `Harborline.Kernel.Audit` manages recovery-event records. Each record carries the recovery mechanism type, the trustee identifiers (where applicable), the claimed identity, the grace-period boundaries, and the completion or dispute attestation.
...
The convergence rule has two layers. **Global log semantics:** no node accepts a completion event for a recovery claim against which a signed dispute already exists in its log; the completion is rejected at validation and re-initiation is required. The rule applies identically to trustee-threshold completions (48a) and custodian-release events (48b). **Local node behavior:** a node that applied a completion before seeing a not-yet-propagated dispute reverses the completion on dispute arrival during sync. The partition window between completion application and dispute arrival is a real architectural fact; security-sensitive deployments size the grace period to make it negligible compared to the time required for the original holder to detect and dispute.

The rule is asymmetric by design. The original holder's authority to halt outweighs the trustees' or custodian's authority to complete, because the cost of an erroneously-completed recovery is permanent loss of the original holder's data control while the cost of a halted-and-re-initiated recovery is operational delay. `Harborline.Foundation.Recovery` enforces this convergence at the audit-log validation layer; the local-reversal behavior is a sync-layer consequence of the partition tolerance the architecture commits to elsewhere.
...
Nodes wishing to integrate recovery flows subscribe to the relevant contracts through `Harborline.Kernel.Audit`. The audit-log validation layer enforces the convergence rules before any UX layer observes the events.
...
<!-- code-check: this section references two Harborline namespaces - `Harborline.Kernel.Security` and `Harborline.Kernel.Sync` - both already in the current Harborline package canon. No new top-level namespace is introduced; forward secrecy and post-compromise security extend the existing kernel session layer rather than adding a new Foundation tier. -->
...
This is the symmetric input path that sub-pattern 46b advances. `Harborline.Kernel.Sync` owns the ephemeral exchange at session establishment. `Harborline.Kernel.Security` owns the HKDF derivation and the per-message zeroing. The handshake boundary is the sync daemon protocol defined in Ch14 §Sync Daemon Protocol.
...
Per-message ephemeral keys prevent retrospective decryption. The Double Ratchet pattern (Marlinspike and Perrin, Signal Foundation, 2016) [5] adds healing - the property that the session recovers confidentiality after a compromise without operator intervention.
...
Session establishment uses an asynchronous key agreement protocol. The Signal X3DH (Extended Triple Diffie-Hellman) construction [6] establishes the initial shared secret from prekeys published in advance, allowing one party to initiate a session while the other party is offline. The Double Ratchet then runs over the X3DH output. The Inverted Stack adopts X3DH or an equivalent asynchronous key agreement so that a node coming online can begin a session with a currently offline peer.

The Noise framework [7] provides a composable substrate for handshake construction. The KK pattern (both parties have known static keypairs) is the closer fit to the Inverted Stack's enrolled-device model, where each node holds a registered device keypair. The architecture does not mandate one specific construction; it mandates that whatever construction is used satisfies the two properties named above. MLS (Messaging Layer Security, RFC 9420) [8] extends ratcheting to group messaging through a TreeKEM construction; deployments with large role groups may adopt MLS in place of pairwise Double Ratchet. The WhatsApp end-to-end encryption specification [9] documents the Double Ratchet at billion-user scale.

`Harborline.Kernel.Sync` extends its session layer to implement the Double Ratchet, an MLS group session, or a Noise-pattern equivalent. The architectural commitment is the two properties - forward secrecy and post-compromise security - not the specific construction. Cross-reference to Ch14 §Sync Daemon Protocol for the handshake that carries the ratchet state; Ch15 §Relay Trust Model for the relay's role as ciphertext forwarder that observes ratchet messages without decrypting them.
...
`Harborline.Kernel.Security` exposes the rotation trigger interface. `Harborline.Kernel.Sync` executes the new ephemeral exchange at the next feasible session event. The trigger is a no-op if the DH ratchet has already advanced within the rotation window through normal message traffic.
...
[2] Argent, "Argent Smart Wallet Specification," *github.com/argentlabs*, 2020. [Online]. Available: https://github.com/argentlabs/argent-contracts/blob/develop/specifications/specifications.pdf

[3] V. Buterin, "Why we need wide adoption of social recovery wallets," *vitalik.ca*, Jan. 2021. [Online]. Available: https://vitalik.ca/general/2021/01/11/recovery.html

[4] Apple Inc., "Apple Platform Security," May 2024. [Online]. Available: https://support.apple.com/guide/security/welcome/web

[5] M. Marlinspike and T. Perrin, "The Double Ratchet Algorithm," Signal Foundation, Nov. 2016. [Online]. Available: https://signal.org/docs/specifications/doubleratchet/

[6] M. Marlinspike and T. Perrin, "The X3DH Key Agreement Protocol," Signal Foundation, Nov. 2016. [Online]. Available: https://signal.org/docs/specifications/x3dh/

[7] T. Perrin, "The Noise Protocol Framework," rev. 34, Jul. 2018. [Online]. Available: https://noiseprotocol.org/noise.html

[8] R. Barnes, B. Beurdouche, R. Robert, J. Millican, E. Omara, and K. Cohn-Gordon, "The Messaging Layer Security (MLS) Protocol," Internet Engineering Task Force, RFC 9420, Jul. 2023. [Online]. Available: https://www.rfc-editor.org/rfc/rfc9420

[9] WhatsApp Inc., "WhatsApp Encryption Overview - Technical White Paper," Nov. 2021. [Online]. Available: https://www.whatsapp.com/security/WhatsApp-Security-Whitepaper.pdf

[10] N. Borisov, I. Goldberg, and E. Brewer, "Off-the-Record Communication, or, Why Not To Use PGP," in *Proc. ACM Workshop on Privacy in the Electronic Society (WPES)*, Washington, DC, USA, Oct. 2004, pp. 77–84.

[11] Sigstore Project, "Rekor: Transparency Log for Software Supply Chains," Linux Foundation, 2023. [Online]. Available: https://docs.sigstore.dev/logging/overview/

[12] J. Van Bulck *et al.*, "Foreshadow: Extracting the Keys to the Intel SGX Kingdom with Transient Out-of-Order Execution," in *Proc. 27th USENIX Security Symposium*, 2018, pp. 991–1008.

[13] S. van Schaik *et al.*, "SGAxe: How SGX Fails in Practice," 2020. [Online]. Available: https://sgaxe.com/
...
[17] C. Dwork and A. Roth, "The Algorithmic Foundations of Differential Privacy," *Foundations and Trends in Theoretical Computer Science*, vol. 9, nos. 3–4, pp. 211–487, 2014. [Online]. Available: https://www.cis.upenn.edu/~aaroth/Papers/privacybook.pdf

[18] A. Machanavajjhala, D. Kifer, J. Gehrke, and M. Venkitasubramaniam, "l-Diversity: Privacy Beyond k-Anonymity," *ACM Transactions on Knowledge Discovery from Data*, vol. 1, no. 1, Mar. 2007. [Online]. Available: https://dl.acm.org/doi/10.1145/1217299.1217302
