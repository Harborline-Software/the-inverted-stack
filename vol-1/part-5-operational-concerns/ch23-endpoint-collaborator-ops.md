...
<!-- code-check: namespaces inherited from Ch15. Harborline.Kernel.Security, Harborline.Kernel.Sync, Harborline.Kernel.Audit, Harborline.Kernel.Custody (forward-looking from #9), Harborline.Foundation.Catalog (forward-looking from #10). Per-section code-check annotations carried with their sections. -->
...
A field engineer's laptop spent the weekend in airplane mode in a hotel safe. On Monday morning, the engineer opens the laptop in the office and lets it reconnect. Friday afternoon - three days earlier, while the laptop was offline - the administrator revoked the engineer's role: a routine project rotation, not an incident. The relay knows this. The laptop does not.

A node that is offline when a revocation event occurs does not receive that event until reconnection. The relay enforces revocation at the handshake layer, not through an out-of-band push. For deployments where intermittent connectivity is baseline operating cadence - major African enterprise field operations, rural Indian BFSI, LATAM commercial-vehicle telematics, GCC remote-site construction - this primitive is daily, not exceptional.
...
<!-- code-check: this section references three Harborline namespaces. `Harborline.Kernel.Security` and `Harborline.Kernel.Audit` are in the current Harborline package canon (verified 2026-04-28: packages/kernel-audit/ exists). `Harborline.Foundation.Recovery` is forward-looking under ADR 0046 - referenced here for the successor-entity KEK separation in the dissolution scenario; illustrative in the same sense the book's existing pre-1.0 Harborline references are illustrative. -->
...
5. `Harborline.Kernel.Security` discards the old KEK after all authorized members confirm receipt of the new bundle. The revoked node, which never received the new bundle, cannot decrypt documents re-wrapped under the new KEK.
...
The architecture does not and cannot delete data from a remote device. Deployments that require demonstrated destruction - regulated industries, GDPR Article 17 obligations and the parallel deletion rights under DPDP Section 12, Brazil LGPD Article 18, UAE DPL Article 16, DIFC DPL 2020 Chapter 4, POPIA Section 24, NDPR, Kenya DPA, Japan APPI Article 36, South Korea PIPA, and Mexico LFPDPPP ARCO cancellation; data-subject deletion requests where a former employee was the data subject - enforce destruction through MDM policies, device-wipe procedures, and contractual obligations. The audit trail (sub-pattern 45f) documents the data-at-risk window and the frozen-cache timestamp. Cross-reference to Ch15 §GDPR Article 17 and Crypto-Shredding for the content-erasure obligations on the originator's side; cached-copy management on the revoked party's device is a separate obligation.
...
4. `Harborline.Foundation.Recovery` generates a fresh KEK for each successor entity. Each party re-encrypts their successor log under that new KEK. The other party's old KEK cannot decrypt the successor log going forward. `Harborline.Kernel.Security` enforces the cryptographic separation once the boundary is declared.
...
CRDT operation-identifier semantics in both engines the architecture targets confirm the partition is safe. Yjs assigns every operation an `(clientID, clock)` pair where `clientID` is a random 53-bit integer chosen on first insert and `clock` is a per-client monotonic counter [4]. Loro uses the same shape: `ID = (PeerID, Counter)` where `PeerID` is a random `u64` and `Counter` is a per-peer monotonic `i32` [5].
...
`Harborline.Kernel.Audit` records every revocation event, every key rotation triggered by revocation, every partition authorization, and every revocation dispute as a signed entry in the encrypted audit log.
...
The audit trail is the legally defensible record of when access ended. For employment disputes, it documents when the deployment terminated the former employee's data access. For partnership dissolutions, it is the partition authorization record. For regulated industries - HIPAA, SOX, PCI-DSS, NIST SP 800-12 [6], DIFC DPL 2020, India DPDP Act 2023 + RBI BFSI circular, China PIPL + MLPS 2.0, Japan PIPA, South Korea PIPA + ISMS-P, Russia 242-FZ, NDPR, POPIA - it is the access-termination artifact those frameworks require. The trail composes with the recovery-event audit trail in Ch22 §Key-Loss Recovery sub-pattern 48f; both share the `Harborline.Kernel.Audit` substrate while their record schemas remain distinct. Cross-reference to §Chain-of-Custody for Multi-Party Transfers (in this chapter) for the multi-party signed-event substrate that underpins both.
...
The threat model includes two distinct adversaries the chapter must name. The first is the conventional endpoint attacker - a malicious actor who has compromised the OS through phishing, supply-chain implant, or zero-click exploit. The second is the state-mandated access scenario - operating environments where infrastructure-layer access is compelled by legal or political authority, requiring end-to-end encryption with local key management as the architectural response rather than an optional posture. The mechanisms in this chapter address both: the relay never holds plaintext, keys never leave originating devices unencrypted, and the architectural reasoning that protects against an OS-level compromise also protects against an infrastructure-level compulsion order. Volkov's CIS-deployment context surfaced this distinction; the architecture has always answered it, but the chapter now states it directly.
...
The architecture does not claim to solve endpoint compromise. It claims four things: blast-radius containment via the three mechanisms in §47e; hardware-backed key protection where the platform offers it; a remote-wipe path that completes when the device is reachable; and honest documentation of the residual risk so practitioners plan against it rather than discover it in an incident post-mortem. Pegasus, Predator, and Hermit operate at the level of full OS compromise with zero-click delivery [14][15]. Against these, hardware enclave separation is the only control that reliably retains key protection. On a fully Pegasus-compromised device, keys in the OS keychain are accessible; keys in a hardware enclave are not. No software-only architecture can claim otherwise.

The honesty extends beyond endpoint compromise to vendor-dependency risk. In 2022, large-scale Western SaaS service terminations across the CIS region - Adobe, Autodesk, Figma, Microsoft - left organisations unable to access keys, files, and project state held inside vendor systems they could no longer reach. For cloud-key-custody architectures, that case is unrecoverable: when the vendor goes silent, the keys go silent with the vendor. The local-first architecture this chapter operates over does not face that scenario at all. User-held keys remain user-held when a vendor terminates service or exits a market, and the operational flows in this chapter - revocation, custody, re-classification - continue working against a fleet that no longer reaches the original software publisher. That is the largest documented demonstration of the failure mode local-first key custody is built to address, and 47f's commitment to honest documentation extends to naming it. Local-first also preserves an accessibility property the rest of this chapter takes for granted: assistive technology operates on the local data source independent of connectivity, vendor reachability, or relay availability - a screen-reader user, a low-vision user using OS-level magnification, or a user on a degraded-connectivity link continues to function during every revocation, custody handoff, and re-classification flow this chapter specifies.
...
Any FAILED condition confirmed at technical review escalates to `Harborline.Kernel.Security` maintainers before the draft advances. This primitive has security-boundary implications; a confirmed failure is not a prose-pass defect.
...
<!-- code-check annotations: Harborline.Kernel.Custody (NEW namespace, not in canon - forward-looking); Harborline.Kernel.Audit (in-canon per cerebrum 2026-04-28, packages/kernel-audit/ exists). 0 class APIs / method signatures introduced. -->
...
sequenceDiagram
    ...
    participant Audit as Harborline.Kernel.Custody<br/>+ Harborline.Kernel.Audit
...
Chain-of-custody is not key-loss recovery. Recovery handles the case where a legitimate user loses custody of their own key; chain-of-custody handles the case where data moves from one authoritative party to another with each transition attested. The two share `Harborline.Kernel.Audit` as substrate but their event schemas, authorization parties, and legal uses are distinct.
...
The version vector matters because CRDT documents evolve. A dashcam frame captured at 14:07:31 and exported to a custody record at 14:07:35 must be identifiable as the specific state at those moments - not "the dashcam recording in general." The CRDT vector clock specifies the exact set of applied operations defining the transferred version, independent of any application-layer name [4][5].
...
The architecture addresses this with an append-only Merkle-tree commitment over the event stream, following the construction Crosby and Wallach formalized for tamper-evident logging [18] and standardized for the web PKI in Certificate Transparency [19]. As each event lands in the export log, the export pipeline folds its hash into a running Merkle root. The pipeline signs the root and emits it alongside each event batch. The regulator verifies that the received stream matches the committed root and that the root chain is internally consistent - each new root extends the prior one by exactly the new events, with no gaps. Any omission or reordering breaks the Merkle chain, and the regulator detects the gap without requiring any cooperation from the operator.
...
Chain-of-custody is observable through four named event contracts. The list is illustrative; the concrete schema lands when `Harborline.Kernel.Custody` reaches its first milestone.
...
<!-- code-check annotations: Harborline.Kernel.Security (in-canon, extends existing); Harborline.Kernel.Audit (in-canon per cerebrum 2026-04-28); Harborline.Kernel.SchemaRegistry (in-canon); Harborline.Kernel.Sync (in-canon, propagates re-classification operation via gossip). 0 new top-level namespaces. 0 class APIs / method signatures introduced. -->
...
Microsoft Purview sensitivity labels [20], AWS Macie, and Google Cloud DLP all perform classification and re-classification as centralized, online operations against a canonical store. The architecture's contribution is the operation that does the same job *under partial replication, without duplicating storage, without breaking the immutable audit trail, and without requiring the central store those systems assume*. The mechanism is convergent re-classification - an idempotent CRDT operation that propagates through the same gossip path every other operation uses, with a max-register invariant guaranteeing every replica reaches the same class label regardless of operation arrival order.
...
The CRDT invariant governing class level is the **max-register**. The class label of a record at any node is `max(L₁, L₂, ..., Lₙ)` taken over every re-classification operation that has reached the node. Max is associative, commutative, and idempotent - the three properties a strict semilattice requires for monotonic convergence. Two nodes that receive the same operations in different orders converge to the same label; a replayed operation does not change the resulting class. The mechanism aligns with the high-watermark principle NIST SP 800-60 prescribes for aggregated systems [21][22] and the periodic-review obligation ISO/IEC 27001:2022 Annex A 5.12 places on classified information [23].

The invariant has a sharp edge. **An operation `(record_id, lower_class)` is rejected at every replica.** Re-classification is monotonic upward; a record at Class 3 cannot be returned to Class 2 by any authority. A downward move would re-grant access already cut off; if a deployment determines an escalation was issued in error, the corrective path is deletion-and-recreation at the lower class with a fresh record identifier. The trigger event identifier is the load-bearing field for accountability - every operation names its cause (an incident-flag, a legal-hold directive, a regulator notice, an inferred-special-category trigger fired when a content scanner detects a GDPR Article 9 attribute [24]) and resolves to a record in `Harborline.Kernel.Audit`.

**The contribution stated precisely.** Max-register itself is an established CRDT primitive (Roh et al., Shapiro et al.). The contribution is the *application* of max-register to a security-metadata field with delivery-side access-control re-evaluation under partial replication - convergent re-classification on a security label without a canonical store, where every node enforces access at delivery against its locally-converged class. The Purview/Macie/DLP contrast above identifies the architectural gap this fills.

### Sub-pattern 10b - Backward propagation across replicated copies

A re-classification operation propagates through the gossip path the sync daemon uses for every operation in `Harborline.Kernel.Sync`. The mechanism is structurally identical to the revocation broadcast specified in §Collaborator Revocation and Post-Departure Partition. Where revocation broadcasts "this collaborator's KEK share is revoked," re-classification broadcasts "this record's class level has changed, and the new class's access policy applies retroactively." Transport identical; asserted fact different.

What distinguishes re-classification from an ordinary edit is the receiving node's behavior on delivery. The sync daemon hands the operation to `Harborline.Kernel.Security` for immediate access-control re-evaluation - not on the next refresh cycle, not at the next session handshake, but inline before any pending read or write on the affected record reaches the application layer. Roles that held access at the prior class but lack access at the new class see their next attempted read denied with a `class-escalated` reason code.
...
The record's operation history was written under the prior class. Truncating it to hide pre-escalation state is out of scope - it would break the CRDT's append-only invariant. The historical operations remain in the log; the access policy that governs their retrieval changes. A role authorized at Class 1 was authorized to read history at Class 1; after escalation to Class 3, that role is not authorized to read the history unless it also holds access at Class 3. Cached copies a node held before escalation remain locally readable on that node - see §Collaborator Revocation sub-pattern 45c for the cached-copy framework - but no further history can be pulled from peers under the prior credential. The escalation event itself produces a record in `Harborline.Kernel.Audit` carrying the record identifier, prior and new class levels, trigger identifier, asserting authority, and UTC timestamp; the audit record's own class is the **new** class.

### Sub-pattern 10d - Cross-class references and operator review

A low-class record holding a reference to a high-class record carries an implicit class obligation. The reference does not reveal content, but it proves a relationship exists - in a legal-hold context, a routine note referencing an incident report's identifier may disclose the existence of the investigation. When a record escalates, `Harborline.Kernel.Security` queries the reference index maintained by `Harborline.Kernel.SchemaRegistry` for every record holding an inbound reference to the escalated identifier. The audit trail flags those records for operator review; they are **not** auto-escalated, because auto-lifting would cascade unbounded. The operator either confirms referencing records remain at their original class or issues a separate re-classification operation, which propagates through 10b as a first-class escalation. The review surface is specified in Ch20 §Data-Class Escalation UX.

### Sub-pattern 10e - Schema-evolution non-interaction

Class level is a metadata field on the record's envelope, not a schema field on its payload. Re-classification does not change payload shape, does not require a lens, and does not advance the schema version; the migration engine is not invoked. Per-class field-access rules - fields permitted at Class 1 that must be redacted at Class 3 - are enforced at read time by `Harborline.Kernel.Security`, not by the schema migration engine. Schema migration mutates payload shape; access enforcement filters payload content for delivery.
...
- **The escalation event is not recorded in `Harborline.Kernel.Audit` at the new class level.** Compliance failure.
...
The five operations this chapter specifies - offline-node revocation, collaborator revocation, endpoint compromise, multi-party custody, event-triggered re-classification - share a single rhetorical commitment: each names what the primitive does, what it does not, what fails if it fails, and the kill trigger that retires it from the conformance set. Chapter 22 specified the operations the key lifecycle drives; this chapter specified the operations the world drives. The next chapter takes one further step away from cryptography and into the physical infrastructure these primitives ride on.

---

## References

[1] Internet Engineering Task Force (IETF), "OAuth 2.0 Token Revocation," RFC 7009, Aug. 2013. [Online]. Available: https://www.rfc-editor.org/rfc/rfc7009

[2] Internet Engineering Task Force (IETF), "X.509 Internet Public Key Infrastructure Online Certificate Status Protocol - OCSP," RFC 6960, Jun. 2013. [Online]. Available: https://www.rfc-editor.org/rfc/rfc6960

[3] Internet Engineering Task Force (IETF), "Internet X.509 Public Key Infrastructure Certificate and Certificate Revocation List (CRL) Profile," RFC 5280, May 2008. [Online]. Available: https://www.rfc-editor.org/rfc/rfc5280

[4] K. Jahns, "Yjs Internals - Document Structure and Item Identifiers," *yjs/yjs* repository, 2024. [Online]. Available: https://github.com/yjs/yjs/blob/main/INTERNALS.md

[5] Loro Project, "Loro Common - Operation ID Type Definitions," *loro-dev/loro* repository, 2024. [Online]. Available: https://github.com/loro-dev/loro/blob/main/crates/loro-common/src/lib.rs

[6] National Institute of Standards and Technology (NIST), "An Introduction to Computer Security: The NIST Handbook," SP 800-12 Rev. 1, Oct. 1995 (rev. 2017). [Online]. Available: https://csrc.nist.gov/publications/detail/sp/800-12/rev-1/final

[7] Apple Inc., "Apple Platform Security," May 2024. [Online]. Available: https://support.apple.com/guide/security/welcome/web

[8] Google, "Pixel Titan M and Android Hardware-Backed Keystore," Android Developers documentation, 2024. [Online]. Available: https://source.android.com/docs/security/features/keystore and https://developer.android.com/privacy-and-security/keystore

[9] Microsoft, "Microsoft Pluton security processor," Microsoft Security Blog, Nov. 17, 2020. [Online]. Available: https://www.microsoft.com/en-us/security/blog/2020/11/17/meet-the-microsoft-pluton-processor-the-security-chip-designed-for-the-future-of-windows-pcs/

[10] J. Van Bulck *et al.*, "Foreshadow: Extracting the Keys to the Intel SGX Kingdom with Transient Out-of-Order Execution," in *Proc. 27th USENIX Security Symposium*, 2018, pp. 991–1008.

[11] K. Murdock *et al.*, "Plundervolt: Software-based Fault Injection Attacks against Intel SGX," in *Proc. IEEE Symposium on Security and Privacy (S&P)*, 2020, pp. 1466–1482.

[12] S. van Schaik *et al.*, "SGAxe: How SGX Fails in Practice," 2020. [Online]. Available: https://sgaxe.com/

[13] Arm Ltd., "Arm Security Technology - Building a Secure System using TrustZone Technology," white paper, Apr. 2009 (rev. 2022). [Online]. Available: https://developer.arm.com/documentation/prd29-genc-009492/

[14] Amnesty International Security Lab, "Forensic Methodology Report: How to catch NSO Group's Pegasus," Jul. 2021. [Online]. Available: https://www.amnesty.org/en/latest/research/2021/07/forensic-methodology-report-how-to-catch-nso-groups-pegasus/ (cross-confirmed by Citizen Lab peer review).

[15] Lookout Threat Intelligence, "Lookout Discovers Hermit Spyware," Apr. 2022 (with subsequent Citizen Lab confirmation of Kazakhstan use Jun. 2022). [Online]. Available: https://www.lookout.com/threat-intelligence/article/hermit-spyware-discovery - supplement with Google Threat Analysis Group analysis of Predator/Cytrox at https://blog.google/threat-analysis-group/.

[16] Internet Engineering Task Force (IETF), "Internet X.509 Public Key Infrastructure Time-Stamp Protocol (TSP)," RFC 3161, Aug. 2001. [Online]. Available: https://www.rfc-editor.org/rfc/rfc3161

[17] European Parliament and Council, "Regulation (EU) No 910/2014 on electronic identification and trust services for electronic transactions in the internal market (eIDAS)," Official Journal of the European Union, Jul. 2014, Art. 41. [Online]. Available: https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32014R0910

[18] S. A. Crosby and D. S. Wallach, "Efficient Data Structures for Tamper-Evident Logging," in *Proc. 18th USENIX Security Symposium*, Montreal, Aug. 2009, pp. 317–334. [Online]. Available: https://www.usenix.org/legacy/event/sec09/tech/full_papers/crosby.pdf

[19] Internet Engineering Task Force (IETF), "Certificate Transparency Version 2.0," RFC 9162, Dec. 2021. [Online]. Available: https://www.rfc-editor.org/rfc/rfc9162

[20] Microsoft, "Learn about sensitivity labels - Microsoft Purview," Microsoft Learn documentation, 2024. [Online]. Available: https://learn.microsoft.com/en-us/purview/sensitivity-labels

[21] National Institute of Standards and Technology (NIST), "Guide for Mapping Types of Information and Information Systems to Security Categories," SP 800-60 Vol. 1 Rev. 1, Aug. 2008. [Online]. Available: https://csrc.nist.gov/pubs/sp/800/60/v1/r1/final

[22] National Institute of Standards and Technology (NIST), "Guide for Mapping Types of Information and Information Systems to Security Categories," SP 800-60 Rev. 2 (Initial Working Draft), 2024. [Online]. Available: https://csrc.nist.gov/pubs/sp/800/60/r2/iwd

[23] International Organization for Standardization, "Information security, cybersecurity and privacy protection - Information security management systems - Requirements," ISO/IEC 27001:2022, Annex A 5.12 - Classification of information.

[24] European Parliament, "Regulation (EU) 2016/679 (General Data Protection Regulation)," Official Journal of the European Union, Apr. 2016, Art. 9. [Online]. Available: https://gdpr-info.eu/art-9-gdpr/
