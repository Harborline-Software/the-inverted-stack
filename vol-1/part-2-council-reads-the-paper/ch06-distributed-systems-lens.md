...
The tier mapping has direct regulatory implications. Tier 3 (no-GC) is required - not recommended - for records subject to GDPR (General Data Protection Regulation)/Schrems II, the EU's NIS2 Directive (cybersecurity audit obligations for essential and important entities), India's DPDP (Digital Personal Data Protection) Act + RBI (Reserve Bank of India) financial data localization (where the 90-day business-record tier is structurally insufficient against the 8–10 year transaction-retention requirement and Tier 3 is the correct mapping), the UAE's DIFC (Dubai International Financial Centre) DPL 2020, Japan's APPI (Act on the Protection of Personal Information, 2022 revision), South Korea's PIPA distinct from Japan's APPI, China's PIPL (Personal Information Protection Law) and MLPS 2.0 (Multi-Level Protection Scheme), Brazil's LGPD (Lei Geral de Proteção de Dados), Mexico's LFPDPPP, Colombia's Ley 1581, Nigeria's NDPR, South Africa's POPIA, Kenya's Data Protection Act 2019, and Russia's Federal Law 242-FZ - full matrix in Appendix F. For these regimes, full operation history is a regulatory obligation that no retention optimization can override. The Tier 3 data class is also the structural answer to Schrems II specifically: data retained locally under local key management, with no relay-exposed payload, is the direct architectural response to the constraint on transfers of EU personal data to US cloud providers. GDPR Article 17 (right to erasure) creates a specific tension with full-history retention - a CRDT that retains every operation contains the complete edit history of every field by design. The resolution is deletion via a compensating operation that overwrites content in all present and future reads while preserving the operation log's audit integrity. Chapter 15 (Security Architecture) specifies the mechanism.
...
**Reconnection storm handling.** Gossip anti-entropy pairs nodes randomly and exchanges deltas on a periodic tick. When many nodes reconnect simultaneously, the resource governor throttles per-tick bandwidth consumption so that each gossip cycle processes a bounded number of exchanges. This is not an edge case. Daily load-shedding cycles in Lagos, evening grid-restoration windows in Bogotá and Mumbai tier-2 cities, and rolling outages in rural Kenyan branch deployments produce coordinated reconnection storms as the baseline operating cadence. Relay-outage recovery is one variant of the same pattern. The architecture made this explicit rather than assumed.

A practical note on verification. The convergence guarantees specified above are architectural commitments ahead of the CRDT backend integration currently in progress. The reference implementation replaces its present stub with YDotNet - the .NET port of Yjs ([github.com/yjs/yjs](https://github.com/yjs/yjs), the JavaScript CRDT library) via Rust FFI (Foreign Function Interface) - as the production CRDT engine. Delta apply-back in the sync daemon is the adjacent integration that makes two-node convergence demonstrable end-to-end. The specification is complete. The evidence catches up when those integrations land.

---

## Act 2: Round 2 - Surviving the Correctness Audit

Shevchenko's Round 2 average was 6.8 - slightly below his Round 1 average of 7.1 - with a PROCEED WITH CONDITIONS verdict. His score dropped. That did not signal a failure. It signaled the opposite. The Round 1 architecture had been general enough to critique only at the surface; Round 2 was specific enough to critique at depth, and he took the architecture more seriously in Round 2, not less. The number went down because the engagement went up.
...
A skeptical CTO reading this chapter is entitled to ask why the answer to distributed-systems hardness is to inherit it rather than to delegate it. Spanner, Fauna, and CockroachDB exist precisely so application teams do not have to specify GC tiers, prove split-write windows safe, or write break-glass procedures for corrupt operation sequences. The honest answer is that this architecture does not compete with managed CP databases on operational simplicity. It competes on a constraint they cannot satisfy: the data sovereignty Part I established. A managed CP system places writes through a vendor-controlled coordinator. That coordinator can be subpoenaed, suspended, or compelled to alter behavior by the jurisdiction that hosts it. For deployments where that constraint is binding - regulated data under data-residency obligations, customer endpoints whose plaintext must never reach vendor infrastructure, jurisdictions where Western-cloud dependency is prohibited - managed-CP delegation is not on the menu. The distributed-systems work cannot be outsourced because the trust boundary cannot be moved.

The same argument scales upward. The chapter's correctness story is also a structural answer to *state-mandated infrastructure access* - the threat model that distinguishes deployments where home-jurisdiction compulsion of a vendor's coordinator must not be able to reach customer plaintext. CRDT operations carry their own causal history; local key management keeps plaintext on the endpoint; the relay forwards ciphertext it cannot decrypt. None of those properties survive a managed-CP delegation. They are structural commitments of the architecture this chapter defends.
