...
flowchart TB
    subgraph kernel ["Kernel - stable core"]
        runtime["Harborline.Kernel.Runtime<br/>Node lifecycle · Plugin registry"]
        sync_pkg["Harborline.Kernel.Sync<br/>Gossip daemon · Peer discovery"]
        crdt_pkg["Harborline.Kernel.Crdt<br/>ICrdtEngine abstraction"]
        security_pkg["Harborline.Kernel.Security<br/>Attestation · Key management"]
        buckets_pkg["Harborline.Kernel.Buckets<br/>Partial sync engine"]
        lease_pkg["Harborline.Kernel.Lease<br/>Distributed lease coordinator"]
        schema_pkg["Harborline.Kernel.SchemaRegistry<br/>Schema migration infrastructure"]
    end
...
**Tier 1 - Foundation.** `Harborline.Foundation` provides design tokens and the base type hierarchy. The `SyncState` enumeration - defined in `Harborline.UIAdapters.Blazor` and shared across all UI tiers - encodes five node states: `Healthy` means sync within the freshness threshold; `Stale` beyond threshold but data still trusted for reads; `Offline` no transport; `ConflictPending` requires user resolution before sync proceeds; `Quarantine` flagged for operator review. Chapter 20 §Status Indicators specifies the user-visible representation. Every component in the upper tiers binds to this enumeration. No tier introduces a UI state that does not correspond to a `SyncState` value.

**Tier 2 - Framework-Agnostic Contracts.** `Harborline.UICore` defines the renderer and provider contracts without any dependency on a UI framework. `ISunfishRenderer` accepts a `SunfishWidgetDescriptor` - a framework-agnostic description of a widget including its kind, parameters, and children - and returns an opaque, platform-specific render payload. `ISunfishCssProvider` maps component state to CSS class strings. `ISunfishIconProvider` maps icon identifiers to platform-appropriate icon representations. Components declared at this tier have no Blazor dependency and no React dependency. They describe what a component contract is, not how it renders. This isolation is what allows third-party adapters and compatibility shims to satisfy the same contracts as the production adapters.

**Tier 3 - Adapters and Blocks.** `Harborline.UIAdapters.Blazor` implements Tier 2 contracts as Razor components and ships `ISunfishCssProvider` implementations targeting Bootstrap 5, Fluent UI, and Material 3. Domain plugin blocks live in `Harborline.Blocks.*`: the task board, resource allocation scheduler, ledger posting grid, and bulk conflict resolution wizard are blocks that implement `IUiBlockManifest` and render through Tier 2 component contracts. The sync-state components are a first-class concern at this tier. `SunfishNodeHealthBar` renders three `SunfishSyncStatusIndicator` pills covering the three node health axes: node process health, link status, and data freshness. `SunfishConflictList` subscribes to the kernel conflict inbox. `SunfishFreshnessBadge` binds to per-record staleness metadata maintained by the kernel. `SunfishOptimisticButton` exposes three write states - pending, confirmed, and failed - that map directly to the lifecycle of a locally-applied write awaiting sync confirmation. The `@sunfish/ui-adapters-react` npm package provides parity implementations of the Tier 2 component contracts for applications rendering the node UI surface in a React host.

**Tier 4 - Compatibility and Adapter Layer.** `Harborline.Compat.Telerik` and `Harborline.Compat.Syncfusion` expose the public parameter shapes of their respective component libraries as thin wrappers over canonical Sunfish (the open-source reference implementation, [github.com/ctwoodwa/Sunfish](https://github.com/ctwoodwa/Sunfish)) components. An application migrating from Telerik UI for Blazor replaces `using Telerik.Blazor.Components` with `using Harborline.Compat.Telerik` and retains most markup while routing all rendering through Tier 2 contracts and the Tier 3 CSS provider. This is source-code shape parity, not visual parity: the compat packages do not reproduce the visual appearance of Telerik or Syncfusion components. The same Tier 2 `SyncState` alignment invariant applies equally: a compat wrapper that renders a data grid over stale records must surface `SyncState.Stale` through the same CSS token system as any first-party component.

```mermaid
flowchart TB
    F["Tier 1 - Foundation<br/>Harborline.Foundation<br/>SyncState · Design tokens · Base contracts"]
    U["Tier 2 - Framework-Agnostic Contracts<br/>Harborline.UICore<br/>ISunfishRenderer · ISunfishCssProvider<br/>No UI framework dependency"]
    A["Tier 3 - Adapters and Blocks<br/>Harborline.UIAdapters.Blazor · @sunfish/ui-adapters-react<br/>Harborline.Blocks.*<br/>SunfishNodeHealthBar · SunfishConflictList · SunfishFreshnessBadge"]
    C["Tier 4 - Compatibility Layer<br/>Harborline.Compat.Telerik · Harborline.Compat.Syncfusion<br/>Source-shape parity - SyncState alignment invariant still applies"]
    F --> U
    U --> A
    U --> C
```
...
The architecture's relay-is-ciphertext-only design directly addresses cross-jurisdiction compliance constraints across major regulatory regimes. In every jurisdiction the technical claim is identical: authoritative data and the keys protecting it reside on infrastructure the operator controls; no foreign-operator-accessible payload is created by routing ciphertext through a relay. The relay observes connection patterns, peer pairs, and message timing - that metadata is itself potentially regulated under GDPR Article 4(1) and is mitigated through self-hosted relay deployment specified in Chapter 15.

The most consequential legal instruments cluster into four jurisdictional anchors. **EU/Western Europe:** the 2020 Schrems II ruling (*Data Protection Commissioner v. Facebook Ireland Limited*, CJEU Case C-311/18) constrains cross-border transfer of EU personal data to US-hosted infrastructure without adequate supplemental safeguards; the EU's NIS2 Directive (Article 21 risk-management measures, in force October 2024) and the EU Cyber Resilience Act add cybersecurity-by-design obligations; Germany's BSI publishes the C5 cloud-security catalogue and IT-Grundschutz baseline; France's CNIL is one of 27 EU Data Protection Authorities enforcing GDPR application. **Middle East / South Asia:** the UAE operates three concurrent data protection regimes - DIFC Data Protection Law 2020 (financial-zone), ADGM Data Protection Regulations 2021 (Abu Dhabi free-zone), and the UAE federal Data Protection Law 2022 - each with distinct relay-residency requirements; Saudi Arabia's PDPL 2021 extends the GCC envelope; India's DPDP Act 2023 (consent-based) operates alongside the RBI 2018 BFSI data localization circular (physical residency for payments-related data - distinct compliance instruments). **CIS:** Russia's Federal Law 242-FZ (enacted 2015, predating GDPR by two years) establishes the doctrinal pattern; Kazakhstan and Belarus parallel localization regimes follow it; Russian and CIS public-sector deployments under import substitution (импортозамещение) constraints find the architecture structurally aligned. **East Asia / Africa / Americas:** Japan APPI (2022 revision) distinct from South Korea PIPA + ISMS-P certification; China PIPL alongside MLPS 2.0 (cybersecurity classification scheme parallel to NIS2, not data-localization); Nigeria NDPR (re-enacted 2023), South Africa POPIA, Kenya Data Protection Act 2019, Ghana DPA 2012, ECOWAS Supplementary Act on Personal Data Protection; Brazil LGPD, Mexico LFPDPPP, Colombia Ley 1581, Argentina Ley 25.326. The full coverage matrix sits in Appendix F.

Compliance officers should treat the architecture's structural answer as a primary compliance argument, distinct from and additive to whatever contractual safeguards their jurisdiction requires - including GDPR Article 17 (right to erasure) against the append-only CRDT event log, which Chapter 13 specifies through stream compaction at the schema-evolution layer and Chapter 15 specifies through crypto-shredding at the DEK level. State-mandated infrastructure access - vendor compulsion to suspend service or hand over plaintext under home-jurisdiction order - is a distinct threat model the architecture addresses through local key management; Chapter 15 §Threat Model specifies the architectural response.

## Harborline Package Map

The Harborline package set maps directly to the architectural layers described in this chapter. The dependency direction is strictly from upper layers to lower: plugins depend on kernel packages, kernel packages depend on foundation packages, foundation packages have no Harborline dependencies. No package in a lower layer imports from a higher layer.

| Layer | Package | Primary responsibility |
|---|---|---|
| Foundation | `Harborline.Foundation` | Core contracts, design tokens, identity types, cryptographic primitives |
| Foundation | `Harborline.Foundation.LocalFirst` | Local encrypted store (SQLCipher), `IOfflineStore`, circuit-breaker quarantine queue |
| Kernel contracts | `Harborline.Kernel` | Type-forwarding facade for the seven kernel primitives: entity store, version store, audit log, permission evaluator, blob store, schema registry, event bus |
| Kernel runtime | `Harborline.Kernel.Runtime` | `INodeHost`, `IPluginRegistry`, `ILocalNodePlugin`, `IPluginContext`, all five extension-point contract interfaces |
| Kernel runtime | `Harborline.Kernel.Sync` | `IGossipDaemon`, mDNS peer discovery, handshake protocol, CBOR wire codec, Unix socket and named-pipe transport |
| Kernel runtime | `Harborline.Kernel.Crdt` | `ICrdtEngine` abstraction; YDotNet backend (validated, integration pending); `StubCrdtEngine` (current, test-only, marked "DO NOT SHIP TO PRODUCTION"); Loro backend (aspirational) |
| Kernel runtime | `Harborline.Kernel.Security` | `IAttestationIssuer`, `IAttestationVerifier`, `IRoleKeyManager`, `IEd25519Signer`, `IX25519KeyAgreement` |
| Kernel runtime | `Harborline.Kernel.Buckets` | `IBucketRegistry`, YAML bucket definitions, `IBucketFilterEvaluator`, lazy-fetch stubs |
| Kernel runtime | `Harborline.Kernel.Lease` | Flease-inspired `ILeaseCoordinator` for CP-class record write serialization |
| Kernel runtime | `Harborline.Kernel.Ledger` | Double-entry `IPostingEngine`, CQRS read models, period-close snapshots |
| Kernel runtime | `Harborline.Kernel.SchemaRegistry` | `ISchemaRegistry`, schema lenses, upcaster chains, epoch coordination, stream compaction |
| Kernel runtime | `Harborline.Kernel.EventBus` | `IEventBus`, `IEventLog` with file-backed and in-memory implementations |
| UI | `Harborline.UICore` | `ISunfishRenderer`, `ISunfishCssProvider`, `ISunfishIconProvider`, `SunfishWidgetDescriptor` |
| UI | `Harborline.UIAdapters.Blazor` | Razor component implementations, Bootstrap / Fluent UI / Material 3 CSS providers, sync-state components |
...
| UI compat | `Harborline.Compat.Telerik` | Telerik-parameter-shaped Razor wrappers over canonical Sunfish components |
| UI compat | `Harborline.Compat.Syncfusion` | Syncfusion-parameter-shaped Razor wrappers over canonical Sunfish components |
| Domain blocks | `Harborline.Blocks.*` | Domain plugin building blocks: accounting, assets, forms, inspections, leases, maintenance, rent collection, scheduling, tasks, tax reporting, workflow |
...
The `Harborline.Kernel` facade package exists for a specific structural reason. Five of the seven kernel primitives - entity store, version store, audit log, permission evaluator, and blob store - are already implemented in `Harborline.Foundation.*` sub-packages. `Harborline.Kernel` re-exports them via assembly type forwarding so that a package wanting to communicate "we only touch the kernel surface" takes a single dependency rather than listing individual Foundation sub-packages. The two remaining primitives - schema registry and event bus - are implemented in `Harborline.Kernel.SchemaRegistry` and `Harborline.Kernel.EventBus` respectively and surfaced through stub interfaces in `Harborline.Kernel`. This split between the facade and the runtime package reflects a deliberate boundary: the facade exposes stable contracts; `Harborline.Kernel.Runtime` owns the live services with state.

All Harborline packages are pre-1.0. Package names are stable for reference in this specification; specific interface method signatures and registration-method options are subject to change before the 1.0 release. The Sunfish (the Zone A local-first desktop accelerator) accelerator in `accelerators/anchor/` assembles this full wiring in a working Zone A node. Chapter 17 provides the minimal implementation path using that accelerator as the starting point.

---

## Performance Contracts and Main-Thread Isolation

<!-- code-check: this section references a forward-looking namespace `Harborline.Kernel.Performance` that is part of the Volume 1 extension roadmap and not yet present in the Harborline Shipyard reference implementation. New component and interface names introduced here - `SunfishMergeProgressIndicator`, `PerformanceBudgetValidator`, `IProgressiveDegradation`, and the `PerformanceDegraded` `SyncState` sub-state - are pre-1.0 additions, marked illustrative in their first appearance. The other references - `Harborline.Kernel.Crdt`, `Harborline.UIAdapters.Blazor`, `Harborline.Foundation` - are in the current Harborline Shipyard package canon. -->
...
```csharp
// illustrative - not runnable (Harborline.Kernel.Performance pre-1.0)
...
References to `Harborline.Kernel.Performance` are by package name only; specific method signatures are pre-1.0 and subject to change.
...
The `SyncState` enumeration (Ch11 §The UI Kernel) carries a `PerformanceDegraded` sub-state that the kernel sets when progressive degradation is active on a record or view. UI components in `Harborline.UIAdapters.Blazor` bind to this state through the existing `SunfishNodeHealthBar` indicator architecture; Chapter 20 §Performance Budgets and Progressive Degradation specifies the user-facing surface.
