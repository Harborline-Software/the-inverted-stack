...
**Tier 3 - Managed Relay.** For deployments where direct peer-to-peer connectivity is not viable - restrictive corporate firewalls, mobile devices on carrier NAT, cross-organizational connections - the daemon connects to a managed relay. The relay is a lightweight proxy: it forwards delta streams between peers without decrypting them. All relay-forwarded messages carry the same Ed25519 device-key signatures as direct connections; the receiving daemon verifies each signature before applying any operation, so a compromised relay cannot inject messages nor silently modify them in transit. The relay cannot read content. The relay cannot inject operations. The relay cannot modify operations. The relay protocol is open and the relay role is implementable by any operator the deployment chooses, including self-hosted or in-jurisdiction-resident infrastructure - the trust boundary stays on the operator's side of the wire, satisfying European public-sector vendor-independence requirements as a structural property.

**Deployment context for Tier 3.** Tier 3 is the primary operational tier for several deployment contexts where the architecture is most consequential, not the fallback path: rural India (3G baseline outside Tier-1 cities), rural Sub-Saharan Africa (load-shedding with 3G/2G substrate), rural Latin America (2G-3G outside Tier-1 cities), and GCC (Gulf Cooperation Council) construction sites. African fintech precedents - M-PESA across tens of millions of Kenyan users, MTN MoMo across multiple African markets, FarmerLine's agritech deployments - established the offline-first sync pattern years before the term entered the local-first lexicon; the architecture formalizes what those deployments proved at scale. Hindi-first, Arabic-first, and other non-English primary-language enterprise contexts are first-class deployment targets; the protocol is encoding-agnostic and the application layer carries i18n responsibility.
...
*Reference implementation status.* In the current Harborline Shipyard reference implementation ([github.com/ctwoodwa/Sunfish](https://github.com/ctwoodwa/Sunfish)), delta receipt and vector-clock exchange are wired, but delta apply-back into `ICrdtDocument` is the Wave 2.6 integration work that completes end-to-end convergence. The specification above is the target; the integration gap is named here so readers distinguish protocol behavior from present implementation state.
...
The send-tier invariant operationalizes data minimization requirements under every major regulatory framework the book addresses. The Western anchor is GDPR Article 5(1)(c) plus the 2020 Schrems II ruling (*Data Protection Commissioner v. Facebook Ireland Limited*, CJEU Case C-311/18) constraining transfers of EU personal data to US-hosted infrastructure without adequate supplemental safeguards - the relay routes only ciphertext; local key management means no foreign-operator-accessible payload is created. The UAE's DIFC (Dubai International Financial Centre) DPL 2020 and the parallel ADGM (Abu Dhabi Global Market) Data Protection Regulations 2021 operate in distinct free-zone jurisdictions from the UAE federal Data Protection Law 2022; the relay-residency requirement differs across the three zones, and a deployment must verify which zone the licensed firm operates under before selecting relay infrastructure. India's DPDP Act 2023 and RBI 2018 BFSI data localization circular require financial-sector data to reside on India-resident servers. East Asia: Japan's APPI (Act on the Protection of Personal Information, 2022 revision), South Korea's PIPA (distinct from Japan's APPI) plus ISMS-P certification, China's PIPL (Personal Information Protection Law) and MLPS 2.0 (Multi-Level Protection Scheme). Africa: Nigeria's NDPR (re-enacted 2023), South Africa's POPIA, Kenya's Data Protection Act 2019. CIS: Russia's Federal Law 242-FZ (predating GDPR by two years) and parallel localization regimes in Kazakhstan and Belarus. The full coverage matrix sits in Appendix F. When the relay is deployed within-jurisdiction, the architecture's combination of data minimization and end-to-end encryption satisfies the server-location requirement structurally rather than contractually.

**State-mandated data access - a parallel threat model.** A distinct threat model operates in parallel across multiple jurisdictions: state-mandated data access to cloud-hosted infrastructure under home-jurisdiction compulsion. End-to-end encryption with local key management addresses it architecturally - the relay cannot produce decryptable content under legal compulsion because the relay does not possess decryptable content. This is a deployment consideration relevant across CIS (Commonwealth of Independent States), the EU post-Schrems II, and parts of Asia, not a political judgment about any specific government's policy. Organizations replacing Western SaaS (Software as a Service) under import substitution (импортозамещение) mandates find the architecture directly aligned: no Western cloud infrastructure dependency, optional in-jurisdiction relay, data and keys under local control.
...
Document schemas define subscription scopes within stream definitions registered with `Harborline.Kernel.Sync`. Each stream definition specifies the minimum set of fields required for each role. The daemon uses these scope definitions when constructing outbound deltas: it strips operations for out-of-scope fields before transmitting, even within a stream the receiving node is authorized to receive. The wire mechanism is omission, not redaction - out-of-scope operations are excluded from the delta entirely; no tombstone, placeholder, or "redacted" marker is emitted. A field-level exclusion is invisible to the receiver: the delta simply does not contain operations for that field, and the receiver cannot distinguish absence-by-scope from absence-by-not-yet-written. The typed rejection codes that the daemon emits for handshake failures (`MISSING_ATTESTATION`, `EXPIRED_ATTESTATION`, `INVALID_SIGNATURE`, `SCHEMA_VERSION_INCOMPATIBLE`) are first-class signals available to the application layer and to assistive technology - sync state changes during connectivity loss surface through ARIA live regions on the local data source, not through cloud round-trips that would stall a screen reader.
...
Before a node writes to a CP-class record, it must acquire a distributed lease. The lease grant requires a quorum of peers to acknowledge the request. The default quorum is ceil(N/2) + 1, where N is the current active membership count. The default lease duration is 30 seconds. Each lease token is stamped with the current epoch ID so that a partition cannot produce two subsets each independently believing it has quorum - a stale-epoch lease attempt is rejected by any peer that has advanced past the issuing epoch (specified in Chapter 6 §"Flease split-write proof" and Chapter 11 §"Distributed lease coordination").
...
`Harborline.Kernel.Sync` manages lease coordination. The application layer interacts with leases through the daemon's command channel; it does not implement lease logic directly.
...
Where `base`, `max_seconds`, and `jitter_range` are configurable per deployment. Defaults are `base = 1` second, `max_seconds = 60` seconds (the relay-enforced cap), `jitter_range = 5` seconds. A node that has been offline for an extended period - beyond a full gossip cycle - uses the maximum backoff on its first reconnection attempt regardless of its attempt count. The 3-missed → suspected, 5-missed → failed thresholds for GOSSIP_PING failure detection follow the SWIM protocol pattern (Das, Gupta, Motivala, *Proc. DSN 2002*); the configurable phi-accrual variant (Hayashibara et al.) is supported but not the default.
...
## Integration with Harborline Shipyard

The sync daemon's capabilities are exposed to the application layer through `Harborline.Kernel.Sync`. This package manages the full protocol lifecycle: peer discovery across all three tiers, handshake sequencing, delta computation, gossip scheduling, lease coordination, and reconnection backoff. Stream definitions registered with `Harborline.Kernel.Sync` declare the field-level access rules the daemon enforces during capability negotiation.
...
The separation between `Harborline.Kernel.Sync` and the application layer is not just an API (Application Programming Interface) boundary. It is the boundary that makes the daemon's process isolation meaningful. The application can fail; the network keeps its promises.
