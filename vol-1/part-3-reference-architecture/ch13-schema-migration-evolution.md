...
The reference design for bidirectional schema lenses is Ink and Switch's Cambria project [1]. Cambria demonstrated that lenses can be declared as a data structure rather than hand-written code - a lens schema specifying field renames, type coercions, and structural transforms - and that the lens engine checks the bidirectionality constraint mechanically rather than by inspection. The architecture follows the Cambria model: lenses are registered as `ISchemaLens` implementations via `LensGraph.AddLens()` in `Harborline.Kernel.SchemaRegistry`, and the `LensGraph` engine traverses and composes them. The current implementation requires lenses to be written as C# code rather than as pure declarative specifications - a pragmatic divergence from Cambria's data-driven approach that retains the bidirectionality and graph-composition properties.

### Lens Registration

Each plugin that owns a CRDT document type registers lenses for every version boundary it has published via `LensGraph.AddLens()` in `Harborline.Kernel.SchemaRegistry`. The kernel discovers registered lenses at startup and constructs the version graph. A plugin that registers v1, v2, and v3 but omits the v1-to-v2 lens produces a startup error, not a runtime failure.

`Harborline.Kernel.Sync` uses the kernel's lens registry when negotiating capability with a peer. During the HELLO handshake, the peer reports its schema version. The sync daemon checks whether a path exists through the version graph from the peer's version to the local version. If no path exists, synchronization for the affected record type is suspended until either the peer upgrades or a lens for the missing edge is installed.
...
**Compliance properties of schema history.** The archived epoch streams are forensic artifacts with direct regulatory implications. Under GDPR (General Data Protection Regulation) Article 30 (records of processing activities) and Schrems II (*Data Protection Commissioner v. Facebook Ireland Limited*, CJEU Case C-311/18, 2020) constraint on cross-border transfer of personal data, local-only archival - streams remain on the node operator's storage, not replicated to the managed relay by default - is the structural compliance answer: archival stream retention does not trigger a cross-border transfer event because the data never leaves the jurisdiction. GDPR Article 5(1)(e) storage limitation creates a tension that the configurable retention window addresses - retired epoch streams should be retained only as long as the applicable audit obligation requires, after which the deletion path specified in Chapter 15 (crypto-shredding at the DEK (Data Encryption Key) level) satisfies the right-to-erasure interaction with full-history retention.

The same architectural property answers data localization mandates across the global regulatory envelope. CIS: Russia's Federal Law 242-FZ (predating GDPR by two years) and parallel localization regimes in Kazakhstan and Belarus; the local-only archival property also addresses the state-mandated-infrastructure-access threat model specified in Chapter 15. Middle East / South Asia: UAE DIFC (Dubai International Financial Centre) DPL 2020 + ADGM (Abu Dhabi Global Market) Data Protection Regulations 2021; India's DPDP (Digital Personal Data Protection) Act 2023 with RBI's 2018 BFSI data localization circular requiring archival epoch streams containing payments-related events to remain on India-resident storage. East Asia: Japan APPI (Act on the Protection of Personal Information, 2022 revision); South Korea PIPA (distinct from Japan's APPI) with ISMS-P; China PIPL (Personal Information Protection Law) and MLPS 2.0 (Multi-Level Protection Scheme). Africa: Nigeria NDPR (re-enacted 2023), South Africa POPIA (Section 72 cross-border transfer restrictions), Kenya Data Protection Act 2019. The full coverage matrix sits in Appendix F. In each jurisdiction, the local-only archival property is the structural answer to the cross-border-transfer event the regulation forbids.

**Lens bidirectionality runtime check.** The ISchemaLens bidirectionality invariant is verified at `Harborline.Kernel.SchemaRegistry` startup: each registered lens runs its forward and backward functions against a canonical property-based test fixture; a lens whose round-trip does not produce an identity transformation halts node startup with a diagnostic naming the failing lens edge. Startup halt is unconditional - a node cannot boot in a degraded mode with a partial lens graph, because partial bidirectionality would silently propagate inconsistent state across the wire. This is a runtime assertion, not a code-review convention.
...
**Step 1 - Announce phase.** At the announce phase, the epoch coordinator broadcasts the new schema version as an administrative event. All reachable nodes receive it immediately. Offline nodes receive it on reconnect.

**Step 2 - Lens deployment.** Lens deployment ships with the application update that carries the new schema version. Lens deployment is part of the application release, not a separate operational action. A node that has not installed the update cannot participate in the new schema; it receives the epoch announcement but cannot acknowledge readiness until updated.

**Step 3 - Expand phase open.** The updated application begins dual-writing. Old-schema nodes continue normal operation. New-schema nodes read both fields and prefer the new one.
...
**Step 6 - Quorum check.** The coordinator monitors acknowledgment progress. Nodes that have been offline for extended periods are counted as outstanding. The team administrator can override the quorum wait for a specific peer when the peer is known to be permanently decommissioned - the override requires a signed administrative event citing the peer's role-attestation revocation as evidence, and the override itself is recorded in the audit log as a tamper-evident epoch-quorum-override entry. A peer that returns after an override has been issued must rejoin through the team-onboarding flow rather than through capability negotiation; its prior attestation is no longer valid for the new epoch.

**Step 7 - Contract phase commit.** The coordinator commits the epoch. The sync daemon begins enforcing the minimum version requirement for governed record types. Old-schema nodes that attempt to synchronize receive a version-gate error with a clear upgrade instruction.

**Step 8 - Copy-transform completion.** The background copy-transform job runs and completes. The old epoch stream is retired. Old lens edges are scheduled for removal after the retention window.
...
## Harborline Shipyard (the open-source reference implementation, [github.com/ctwoodwa/Sunfish](https://github.com/ctwoodwa/Sunfish)) Package Reference

| Package | Responsibility |
|---|---|
| `Harborline.Kernel.Runtime` | Schema version registry, `ISchemaVersion` interface, upcaster registration, expand-contract phase tracking |
| `Harborline.Kernel.SchemaRegistry` | Bidirectional lens engine (`ISchemaLens`, `LensGraph`), epoch coordinator, copy-transform migrator, compaction scheduler |
| `Harborline.Kernel.Sync` | Per-peer schema version negotiation via HELLO handshake, lens application on delta transmission and receipt, version-gate enforcement at contract phase |

Both packages are pre-1.0. Interface signatures are illustrative; validate against the current Harborline Shipyard (the open-source reference implementation, [github.com/ctwoodwa/Sunfish](https://github.com/ctwoodwa/Sunfish)) milestone before implementation.
...
[3] *Data Protection Commissioner v. Facebook Ireland Limited and Maximillian Schrems* (Schrems II), Court of Justice of the European Union, Case C-311/18, July 16, 2020.
