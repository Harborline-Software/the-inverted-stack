...
- **[2026-05-21] audiobook.py --check-stale / --only-stale flags.** `--check-stale` is a read-only staleness scan: for each chapter in CHAPTER_FILES, compares source .md mtime to expected MP3 mtime under `OUT_DIR/<vol>/<stem>.mp3`. Prints a table; exits 1 if any stale or missing MP3, 0 if all current. No TTS calls — safe for CI or QM daemon. `--only-stale` auto-renders stale chapters only (implicitly sets --force on the filtered set; skips current chapters). The QM daemon (coordination/qm-daemon.py Check 12 "audio-staleness") calls `--check-stale` as a subprocess and auto-launches `--only-stale` in background when stale chapters found. Lock file `coordination/.audiobook-render.lock` (PID + timestamp + log path) prevents overlapping renders. `--no-audio-autoregen` on the daemon disables the auto-launch (detection-only mode).
...
- [2026-05-21] Do NOT use `sed '/^---$/,/^---$/d'` to strip frontmatter from vol-2 chapters — it breaks silently when the chapter body contains `---` horizontal-rule dividers. The sed command interprets the body `---` as the start of a second frontmatter block and deletes all body text between consecutive `---` lines. Ch03 has two body dividers at raw lines 124 and 190, silently dropping the entire Priya lab scene + Joel scene from any grep count. Correct frontmatter-strip command: `perl -0777 -pe 's/^---\n.*?---\n//s'` (removes only the FIRST `---`...`---` block, as YAML frontmatter always is the first block). Apply this fix to all vol-2 live-grep mandates before reporting counts.
